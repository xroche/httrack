...
Neither side transcodes, and the file declares no encoding, so each front end guesses. WinHTTrack is an ANSI build and writes local-codepage bytes. WebHTTrack writes back whatever the browser posted, which is UTF-8 since 3.50 moved the catalogs and the served pages to it. The two therefore disagree on every non-ASCII value, where before 3.50 they agreed whenever the host codepage matched the catalog's charset. A profile written by one and read by the other keeps its ASCII and loses its accents.
